...
Got to start with the customer experience and work backwards. Not what if the person is going to buy it.
What incredible benefits can we give with the customer.

Not what awesome tech we have and how we're going to market that.

Build the world first something-something... Burning the midnight oil, execute and making decision along the way. We will find the mistakes, and fix them. Support them and see them through this. Mistakes we make, some will be mad. Be it will be in a better place.
...
- [ ] This route needs to be ranned after making a POST /api/users/login

```text
Authorization Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYwYmRhNTI4M2FlYjUwMTMzN2I2MjU3NCIsImVtYWlsIjoidGVzdDE2QG1haWwuY29tIiwibmFtZSI6IlJvbWUiLCJleHBpcmVkVG9rZW4iOjE2MjMwNDE2MDY4OTgsImlhdCI6MTYyMzA0MTYwNiwiZXhwIjoxNjIzMDQ1MjA2fQ.s48-80PeTqztSVza77RUq16UJ2j-vk5t_IJelal5KOw
```
...
| GET | /api/users/test | users.js | Return json |
| POST | /api/users/login | users.js | Login form |
| POST | /api/users/signup | users.js | Signup form |
| GET | /api/users/profile | users.js | Login user |
| GET | /api/users/all-users | users.js | Creates User |
...
```text
npm install dotenv express cors mongoose passport passport-jwt jsonwebtoken axios
```
...
- [jsonwebtoken](https://www.npmjs.com/package/jsonwebtoken): JSON Web Token (JWT) is a compact, URL-safe means of representing
